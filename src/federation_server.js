...
import { StellarTomlResolver } from './zion_toml_resolver';
...
export class FederationServer {
  ...
  resolveAddress(address) {
    ...
    if (address.indexOf('*') < 0) {
      ...
      zionAddress = `${address}*${this.domain}`;
    }
    const url = this.serverURL.query({ type: 'name', q: zionAddress });
