...
import { xdr, StrKey, Asset } from 'zion-base';
...
/**
 * Server handles the network connection to a [Equator](https://www.zion.org/developers/equator/reference/)
 * instance and exposes an interface for requests to that instance.
 * @constructor
 * @param {string} serverURL Equator Server URL (ex. `https://equator-testnet.zion.org`).
...
export class Server {
  constructor(serverURL, opts = {}) {
    ...
    if (this.serverURL.protocol() !== 'https' && !allowHttp) {
      throw new Error('Cannot connect to insecure equator server');
    }
  }
   ...
   * your machine's local time could be different from Equator's. This gives you
   ...
   *
   * ```javascript
   * const transaction = new ZionSdk.TransactionBuilder(accountId, {
   *  fee: await ZionSdk.Server.fetchBaseFee(),
   *  timebounds: await ZionSdk.Server.fetchTimebounds(100)
   * })
   ...
   * This is to avoid a scenario where Equator is horking up the wrong date.
   ...
   * @see [Operation Fee Stats](https://www.zion.org/developers/equator/reference/endpoints/fee-stats.html)
   * @returns {Promise} Promise that resolves to the fee stats returned by Equator.
   */
  operationFeeStats() {
   ...
   *
   * @see [Post Transaction](https://www.zion.org/developers/equator/reference/endpoints/transactions-create.html)
   * @param {Transaction} transaction - The transaction to submit.
   * @returns {Promise} Promise that resolves or rejects with response from equator.
   */
  ...
  /**
   * @returns {AccountCallBuilder} New {@link AccountCallBuilder} object configured by a current Equator server configuration.
   */
  accounts() {
  ...
  /**
   * @returns {LedgerCallBuilder} New {@link LedgerCallBuilder} object configured by a current Equator server configuration.
   */
  ledgers() {
  ...
  /**
   * @returns {TransactionCallBuilder} New {@link TransactionCallBuilder} object configured by a current Equator server configuration.
   */
  transactions() {
  ...
  /**
   * People on the Zion network can make offers to buy or sell assets. This endpoint represents all the offers a particular account makes.
   ...
   * @returns {OrderbookCallBuilder} New {@link OrderbookCallBuilder} object configured by a current Equator server configuration.
   */
  ...
  /**
   * Returns
   * @returns {TradesCallBuilder} New {@link TradesCallBuilder} object configured by a current Equator server configuration.
   */
  trades() {
  ...
  /**
   * @returns {OperationCallBuilder} New {@link OperationCallBuilder} object configured by a current Equator server configuration.
   */
  operations() {
  ...
  /**
   * The Zion Network allows payments to be made between assets through path payments. A path payment specifies a
   ...
   *
   * As part of the search, equator will load a list of assets available to the source address and will find any
   ...
   * @returns {PathCallBuilder} New {@link PathCallBuilder} object configured with the current Equator server configuration.
   */
  ...
  /**
   * @param {string} address The Zion ID that you want Friendbot to send lumens to
   ...
   * Returns new {@link TradeAggregationCallBuilder} object configured with the current Equator server configuration.
